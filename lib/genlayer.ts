...
type Injected = { request: (args: {method:string;params?:unknown[]}) => Promise<any> };
function getInjected() { return typeof window==='undefined' ? null : (window as Window & {ethereum?:Injected}).ethereum ?? null; }
function signingClient() { const provider=getInjected(); const selected=typeof window!=='undefined'&&window.localStorage.getItem('aftercare.walletType')==='injected'; if(selected&&provider){const a=window.localStorage.getItem('aftercare.injectedAddress');return createClient({chain:studionet,account:a as Address,provider});} return getClient(); }
...
export async function waitFor(hash: string) { return signingClient().waitForTransactionReceipt({ hash: hash as never, retries: 120, interval: 3000 }); }
export async function readContract<T>(functionName: string, args: any[] = []) { return signingClient().readContract({ address: requireAddress(), functionName, args }) as Promise<T>; }
export async function writeContract(functionName: string, args: any[] = [], value = BigInt(0)) { const injected=typeof window!=='undefined'&&window.localStorage.getItem('aftercare.walletType')==='injected'; const account=injected?window.localStorage.getItem('aftercare.injectedAddress'):getStoredAccount(); if(!account) throw new Error('Wallet unavailable.'); const hash=await signingClient().writeContract({address:requireAddress(),functionName,args,value,account:account as any}); if(typeof window!=='undefined') window.localStorage.setItem('aftercare.pendingTx',JSON.stringify({action:functionName,hash:String(hash),startedAt:Date.now()})); try{return {hash,receipt:await waitFor(String(hash))};}finally{if(typeof window!=='undefined')window.localStorage.removeItem('aftercare.pendingTx');} }
...
export function activeAddress() { if(typeof window!=='undefined'&&window.localStorage.getItem('aftercare.walletType')==='injected') return (window.localStorage.getItem('aftercare.injectedAddress')||'').toLowerCase(); return getStoredAccount()?.address?.toLowerCase() ?? ''; }
...
export async function connectInjectedWallet() { const provider=getInjected(); if(!provider) return null; const accounts=await provider.request({method:'eth_requestAccounts'}); if(!accounts[0]) return null; window.localStorage.setItem('aftercare.walletType','injected');window.localStorage.setItem('aftercare.injectedAddress',accounts[0]); await createClient({chain:studionet,account:accounts[0] as Address,provider}).connect('studionet'); return accounts[0]; }
